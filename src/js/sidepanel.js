...
// ── STORAGE FACADE ───────────────────────────────────────────────────
//   Settings  → browser.storage.sync   (synced, 8KB/item limit)
//   Chat data → browser.storage.local  (local, ~unlimited quota)
//   UI state  → browser.storage.session (ephemeral per browser session)
const store = {
    getSetting : key => browser.storage.sync.get([key]).then(r => r[key] ?? null),
    setSetting : (key, val) => browser.storage.sync.set({ [key]: val }),
    getLocal   : key => browser.storage.local.get([key]).then(r => r[key] ?? null),
    setLocal   : (key, val) => browser.storage.local.set({ [key]: val }),
    removeLocal: key => browser.storage.local.remove([key]),
    getSession : key => browser.storage.session.get([key]).then(r => r[key] ?? null),
    setSession : (key, val) => browser.storage.session.set({ [key]: val }),
};
...
function exportChat() {
    if (chatHistory.length === 0) { window.showExtensionAlert('No hay mensajes para exportar.'); return; }
