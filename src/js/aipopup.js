...
const safeChromeStorageSetFn = safeChromeStorageSet || ((key, value) => new Promise(resolve => {
    try {
        ...
        console.error('Error setting browser.storage.sync:', error);
        resolve();
    }
}));
    ...
    try {
        ...
        browser.storage.sync.get([key], result => {
            if (browser.runtime.lastError) {
                console.error('Chrome storage local get error:', browser.runtime.lastError);
                resolve(null);
            } else {
...
const safeChromeLocalSet = (key, value) => new Promise(resolve => {
    try {
        ...
        browser.storage.sync.set({ [key]: value }, () => {
            if (browser.runtime.lastError) {
                console.error('Chrome storage local set error:', browser.runtime.lastError);
            }
            resolve();
        });
    } catch (error) {
...
const safeChromeLocalRemove = (key) => new Promise(resolve => {
    try {
        ...
        browser.storage.sync.remove([key], () => {
            if (browser.runtime.lastError) {
                console.error('Chrome storage local remove error:', browser.runtime.lastError);
            }
            resolve();
        });
    } catch (error) {
        console.error('Error removing from browser.storage.sync:', error);
        resolve();
    }
});
...
async function clearChatHistory() {
    ...
    if (confirmed) {
        try {
            ...
            window.showExtensionAlert('Error al borrar el historial. Por favor, inténtalo de nuevo.');
        }
    }
}
...
function exportChatHistory() {
    if (chatHistory.length === 0) {
        window.showExtensionAlert('No hay historial para exportar.');
        return;
    }
